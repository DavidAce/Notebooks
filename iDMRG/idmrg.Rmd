...
\begin{center}
	...
	\toprule
 	...
 	4		&	Update environments	& \idmrgContractLeft {L}{A}{H}{2} $\rightarrow$ \idmrgLeft{L}{2} \hspace{1cm} \idmrgContractRight{R}{B}{H}{2} $\rightarrow$ \idmrgRight{R}{2}   \\
 	5		& Swap $A$ and $B$ &  \\
	%\bottomrule
	\end{tabular}
\end{center}
...
An additional simplification can be made if we work with reduced local Hamiltonians, where we've subtracted the current per-site energy from the diagonal:  $\tilde h_\text{even} = h_\text{even} - \langle \Phi_\text{even} | h_\text{even}| \Phi_\text{even} \rangle$ and similarly for $\tilde h_\text{even}$. Then the same diagrams above will directly give us the per-site variance, i.e. $\text{Var}(H)/L = \langle (\tilde h_\text{even} + \tilde h_\text{odd})^2 \rangle$.
